...
import { ederecoServidor } from '../utils'
...
const Aula15_Login = () => {
    ...
    const botaoEntrar = async () => {
        try {
            ...
            const resposta = await fetch(`${ederecoServidor}/login`, {
                method: 'post',
                headers: {
            ...
            if (resposta.ok) {
                console.log('Login bem sucedido', respostaJson);
                setMensagem('Login bem sucedido')
                localStorage.setItem('Usuario logado', JSON.stringify(respostaJson))
                navigate('/inicio')
            } else {
                setMensagem('Login falhou')
                console.log('Erro ao fazer login', respostaJson);
            }
